...
import { WarcRecord, displayRecord } from "./libs";
...
export const WarcOfflineList = () => {
    const { records, iframeRef } = useContext(WarcOfflineContext);
    ...
    const recordsToList = records?.current.filter(record => record.content !== null && record.uri.includes(search))
                ...
                <div className={styles.singleLineHide}>
                    ...
                    {recordsToList.map((record, index) => (
                        <a href="#"
                            ...
                            onClick={() => {
                                displayRecord(record, iframeRef?.current, { records: records?.current ?? [] })
                            }}>
                            {record.uri}
                        </a>
                    ))}
