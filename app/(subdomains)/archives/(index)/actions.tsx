...
export const getLatestArchived = async () => {
    return await (await fetch(`${process.env.ARCHIVES_INTERNAL_API}/statistics/latest`, {cache: 'no-store'})).json();
}

export const getStatisticsCount = async () => {
    const api = await (await fetch(`${process.env.ARCHIVES_INTERNAL_API}/statistics`, {cache: 'no-store'})).json();
...
export const getFileProgress = async () => {
    return await (await fetch(`${process.env.ARCHIVES_INTERNAL_API}/statistics/progress`, {cache: 'no-store'})).json();
}