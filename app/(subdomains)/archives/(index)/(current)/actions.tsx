...
export const getLatestArchived = async () => {
    ...
    return fetch(`${process.env.ARCHIVES_INTERNAL_API}/statistics/latest`, { next: { revalidate: 60 * 5 } })
        .then(res => res.json());
};
...
export const getStatisticsCount = async () => {
    ...
    */
    ...
    return fetch(`${process.env.ARCHIVES_INTERNAL_API}/statistics`, { next: { revalidate: 60 * 5 } })
        .then(res => res.json())
        .then(api => {
            const views = getCachedViewCount({ documentId: hashDJB2(hashSHA3("/archives/")), uniqueViews: true })();
            return { ...api, totalViews: views };
